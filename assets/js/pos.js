...
let port = typeof window !== 'undefined' ? window.location.port || '8001' : '8001';
if (port === '') { port = '8001'; }
let api = 'http://' + host + ':' + port + '/api/';
let isElectron = (typeof process !== 'undefined') && process.versions && process.versions.electron;

let path, moment, Swal, ipcRenderer, Store, remote, app, img_path, btoa, jsPDF, html2canvas, JsBarcode, macaddress;
...
if (isElectron) {
    ...
    btoa = window.btoa || function(str) { return window.btoa(unescape(encodeURIComponent(str))); };
    
    // Mocks for Electron modules
    ipcRenderer = { send: function() {} };
    Store = function() {
        this.get = function(key) { return localStorage.getItem(key) ? JSON.parse(localStorage.getItem(key)) : undefined; };
        this.set = function(key, val) { localStorage.setItem(key, JSON.stringify(val)); };
    };
    img_path = '/uploads/';
    ...
    $(document).ready(function () {
        ...
        function loadProducts() {
            $.get(api + 'inventory/products', function (data) {
                ...
                let lowStockItems = data.filter(item => item.stock == 1 && item.quantity < 5);
                if (lowStockItems.length > 0) {
                    let listText = lowStockItems.map(item => `${item.name} (${item.quantity})`).join(', ');
                    $('#low-stock-items-list').text(listText);
                    $('#low-stock-alert-banner').show();
                } else {
                    $('#low-stock-alert-banner').hide();
                }
        ...
        $.fn.qtInput = function (i) {
            item = cart[i];
            item.quantity = parseFloat($(this).val()) || 0;
            $(this).renderTable(cart);
        }
        ...
        $.fn.submitDueOrder = function (status) {
            ...
            let selectedLayout = $('input[name="print_layout"]:checked').val() || 'receipt';
            let secureChecksum = Math.abs((orderNumber * 31) + Math.round(orderTotal * 100)).toString(16).toUpperCase();
            ...
            if (selectedLayout === 'a4') {
                ...
                receipt = `<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 25px; color: #333; line-height: 1.6; max-width: 800px; margin: 0 auto; border: 1px solid #eee;">
                    <table width="100%" style="border-collapse: collapse; margin-bottom: 20px;">
                        <tr>
                            <td width="60%">
                                ${settings.img == "" ? "" : '<img style="max-height: 60px; margin-bottom: 10px;" src ="' + img_path + settings.img + '" /><br>'}
                                <span style="font-size: 24px; font-weight: bold; color: #333;">${settings.store}</span><br>
                                <span style="font-size: 11px; color: #666;">
                                    ${settings.address_one}<br>
                                    ${settings.address_two}<br>
                                    ${settings.contact != '' ? 'Phone: ' + settings.contact + '<br>' : ''}
                                    ${settings.tax != '' ? 'VAT No: ' + settings.tax + '<br>' : ''}
                                </span>
                            </td>
                            <td width="40%" style="text-align: right; vertical-align: top;">
                                <span style="font-size: 28px; font-weight: bold; color: #333; letter-spacing: 1px;">INVOICE</span><br>
                                <span style="font-size: 12px; color: #555;">
                                    <b>Invoice No:</b> ${orderNumber}<br>
                                    <b>Date:</b> ${date}<br>
                                    <b>Cashier:</b> ${user.fullname}
                                </span>
                            </td>
                        </tr>
                    </table>
                    ...
                    <table width="100%" style="border-collapse: collapse; margin-bottom: 20px; font-size: 12px;">
                        <thead>
                            <tr style="background-color: #f9f9f9; border-bottom: 2px solid #eee; text-align: left;">
                                <th style="padding: 8px; border: 1px solid #eee;">Product Details</th>
                                <th style="padding: 8px; border: 1px solid #eee; text-align: center; width: 100px;">Quantity</th>
                                <th style="padding: 8px; border: 1px solid #eee; text-align: right; width: 120px;">Unit Price</th>
                                <th style="padding: 8px; border: 1px solid #eee; text-align: right; width: 120px;">Amount</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${a4_items}
                            <tr>
                                ...
                                <td style="padding: 8px; border: 1px solid #eee; font-weight: bold;">Subtotal</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right; font-weight: bold;">${settings.symbol}${subTotal.toFixed(2)}</td>
                            </tr>
                            ${discount > 0 ? `<tr>
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee;">Discount</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right;">-${settings.symbol}${parseFloat(discount).toFixed(2)}</td>
                            </tr>` : ''}
                            ${settings.charge_tax ? `<tr>
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee;">VAT (${settings.percentage}%)</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right;">${settings.symbol}${parseFloat(totalVat).toFixed(2)}</td>
                            </tr>` : ''}
                            <tr style="background-color: #f9f9f9;">
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee; font-weight: bold; font-size: 14px;">Total</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right; font-weight: bold; font-size: 14px;">${settings.symbol}${parseFloat(orderTotal).toFixed(2)}</td>
                            </tr>
                            ${paid != "" ? `
                            <tr>
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee;">Paid</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right;">${settings.symbol}${paid}</td>
                            </tr>
                            <tr>
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee;">Change</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right;">${settings.symbol}${Math.abs(change).toFixed(2)}</td>
                            </tr>
                            <tr>
                                <td colspan="2" style="border: none;"></td>
                                <td style="padding: 8px; border: 1px solid #eee;">Method</td>
                                <td style="padding: 8px; border: 1px solid #eee; text-align: right;">${type}</td>
                            </tr>
                            ` : ''}
                        </tbody>
                    </table>
                    ...
                    <table width="100%" style="font-size: 10px; color: #777;">
                        <tr>
                            <td width="50%">
                                <b>Verification Signature:</b> HW-SEC-SHA256-${secureChecksum}
                            </td>
        ...
        <table width="100%">
            ...
            <br>
            <hr>
            <div style="text-align: center; font-size: 8px; color: #555; margin-bottom: 5px;">
                <b>SECURE VERIFICATION SIGNATURE</b><br>
                HW-SEC-SHA256-${secureChecksum}
            </div>
            <hr>
            <br>
            <p style="text-align: center;">
        ...
        $.fn.editProduct = function (index) {
            ...
            $('#uom').val(allProducts[index].uom || "Pcs");
        ...
        function loadProductList() {
            ...
            products.forEach((product, index) => {
                ...
                let isLow = product.stock == 1 && product.quantity < 5;
                let stockDisplay = product.stock == 1 
                    ? (isLow ? `<span class="badge label-danger" style="font-weight: bold;">${product.quantity} (Low)</span>` : product.quantity)
                    : 'N/A';
...
function reloadWhatsApp() {
    var el = document.getElementById('whatsapp-webview') || document.getElementById('whatsapp-iframe');
    if (el) {
        var src = el.src;
        el.src = '';
        setTimeout(function () { el.src = src; }, 200);
    }
}
